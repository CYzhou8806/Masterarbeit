...
if __name__ == "__main__":
    ...
    dataset_root = r"C:\Users\cyzho\Desktop\kitti_2015\data_scene_flow\test"
    left_root = os.path.join(dataset_root, 'image_2')
    gt_root = left_root.replace('image_2', 'disp_occ_0')

    save_root = r"C:\Users\cyzho\Desktop\tmp"
    if os.path.exists(save_root):
        shutil.rmtree(save_root)
    os.mkdir(save_root)
    mask_tex_path = os.path.join(save_root, 'mask_textureless')
    os.mkdir(mask_tex_path)
    mask_disc_path = os.path.join(save_root, 'mask_discont')
    os.mkdir(mask_disc_path)
    mask_occ_path = os.path.join(save_root, 'mask_occlusions')
    os.mkdir(mask_occ_path)
    mask_indicator_path = os.path.join(save_root, 'mask_indicator')
    os.mkdir(mask_indicator_path)

    for root, dirs, files in os.walk(left_root):
        for file in  tqdm(files):
            file_name = os.path.basename(file)
            left_image = imread(os.path.join(root, file))
            gt_disparity = imread(os.path.join(root, file).replace('image_2', 'disp_occ_0'))

            mask_tex = compute_textureless(left_image, textureless_width, textureless_thresh)
            imwrite(os.path.join(mask_tex_path, file_name), mask_tex)

            mask_disc = compute_disparity_discont(gt_disparity, disp_gap, discont_width)
            imwrite(os.path.join(mask_disc_path, file_name), mask_disc)

            mask_occ = compute_occluded(gt_disparity)
            imwrite(os.path.join(mask_occ_path, file_name), mask_occ)

            mask_indicator = compute_good_region(gt_disparity, mask_tex, None, mask_occ)
            imwrite(os.path.join(mask_indicator_path, file_name), mask_indicator)