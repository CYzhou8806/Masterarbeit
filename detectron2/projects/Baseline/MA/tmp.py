...
disparity_file_name = r"C:\Users\cyzho\Desktop\LUIS\000000_10.png"
dis_gt = Image.open(disparity_file_name)
tmp = np.array(dis_gt)
dis_gt = utils.read_image(disparity_file_name, "RGB")

print(np.all(dis_gt[:, :, 0] == dis_gt[:, :, 1]))
print(np.all(dis_gt[:, :, 0] == dis_gt[:, :, 2]))
print(np.all(dis_gt[:, :, 1] == dis_gt[:, :, 2]))

print(np.all(dis_gt[:, :, 0] == 0))
