...
def get_kitti360_files(image_dir, gt_dir, json_info):
    output = []
...
def load_kitti360(image_dir, pan_gt_dir, pan_gt_json, meta):
    ...
    assert os.path.exists(
        pan_gt_json
    ), "Please run `python kitti360scripts/preparation/createPanopticImgs.py` to generate label files."  # noqa
    with open(pan_gt_json) as f:
        json_info = json.load(f)
    files = get_kitti360_files(image_dir, pan_gt_dir, json_info)
    ret = []
...
_RAW_KITTI360_SPLITS = {
    "kitti360_train": (
        ...
        "kitti_360/train/panoptic",
        "kitti_360/train/panoptic.json",
    ),
}
...
def register_all_kitti360(root):
    ...
    thing_classes = [k["name"] for k in KITTI360_CATEGORIES]
    thing_colors = [k["color"] for k in KITTI360_CATEGORIES]
    stuff_classes = [k["name"] for k in KITTI360_CATEGORIES]
    stuff_colors = [k["color"] for k in KITTI360_CATEGORIES]
    ...
    for key, (image_dir, right_img_dir, dis_gt_dir, pan_gt_dir, gt_json) in _RAW_KITTI360_SPLITS.items():
        ...
        DatasetCatalog.register(
            key, lambda x=image_dir, y=pan_gt_dir, z=gt_json: load_kitti360(x, y, z, meta)
        )
